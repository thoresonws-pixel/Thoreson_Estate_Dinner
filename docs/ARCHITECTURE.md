...
1. **Authorization and privacy:** production RTDB rules currently allow signed-in users broad game access, and user-owned profile roles are unsafe as an authority source. Introduce server-managed administrator grants, enforce membership/host boundaries, split public game projections from private player records, and test denied requests with real emulator credentials. Update every legacy whole-game reader before tightening reads; rules cannot hide a child after granting a parent read. This is a release blocker, not a cosmetic task.
