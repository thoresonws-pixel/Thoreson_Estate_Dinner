...
This establishes the personal-inventory boundary. The next pass adds triggered-memory privacy, validated billiards results and piano reservations; see [memory and actions](MEMORY_AND_ACTIONS.md). Shared game reads, remaining character fields, membership and several other puzzle commands are still release gates. Do not describe the whole game as cheat-proof or production-secure.
