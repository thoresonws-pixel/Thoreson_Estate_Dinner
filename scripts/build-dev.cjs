...
for(const e of fs.readdirSync(root,{withFileTypes:true}))if(e.isFile()&&/\.(html|js|css|ico)$/.test(e.name)&&!/(archive|backup|migrate|sw\.js)/.test(e.name)){const staging=path.join(out,e.name);let s=fs.readFileSync(path.join(root,e.name),'utf8');s=s.replace(/firebase\.initializeApp\(\s*(?:firebaseConfig|\{[\s\S]*?\})\s*\)/g,'firebase.initializeApp('+JSON.stringify(config)+')');if(e.name.endsWith('.html'))s=s.replace('</head>','<script>if(navigator.serviceWorker)navigator.serviceWorker.register=()=>Promise.reject(Error("Disabled in development"));</script></head>');fs.writeFileSync(staging,s);}
for(const name of ['stories','assets','images','css','js'])if(fs.existsSync(path.join(root,name)))copy(path.join(root,name),path.join(out,name));
